/**
 * Explicit Visibility Rule Tests
 *
 * Testing enforcement of explicit visibility modifiers for functions
 */

import { ExplicitVisibilityRule } from '@/rules/lint/explicit-visibility';
import { AnalysisContext } from '@core/analysis-context';
import { SolidityParser } from '@parser/solidity-parser';
import { Severity, Category } from '@core/types';
import type { ResolvedConfig } from '@config/types';

describe('ExplicitVisibilityRule', () => {
  let rule: ExplicitVisibilityRule;
  let parser: SolidityParser;
  let config: ResolvedConfig;

  beforeEach(() => {
    rule = new ExplicitVisibilityRule();
  ...
  describe('metadata', () => {
    test('should have correct metadata', () => {
      expect(rule.metadata.id).toBe('lint/explicit-visibility');
  ...
  describe('Function visibility', () => {
    ...
    test('should report issue for function without visibility modifier', async () => {
      ...
      expect(issues[0]?.ruleId).toBe('lint/explicit-visibility');
      expect(issues[0]?.severity).toBe(Severity.WARNING);
      expect(issues[0]?.message).toContain('explicit visibility');
